...
func TestBTreePut3(t *testing.T) {
	...
	tree := NewWithIntComparator[int, int](6)
	assertValidTree(t, tree, 0)
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	...
	if actualValue, expectedValue := strings.Trim(strings.Replace(fmt.Sprint(tree.Keys()), " ", "", -1), "[]"), "1234567"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := strings.Join(tree.Values(), ""), "xbcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue := tree.Size(); actualValue != 7 {
		t.Errorf("Got %v expected %v", actualValue, 7)
...
func assertValidTree[TKey, TValue comparable](t *testing.T, tree *Tree[TKey, TValue], expectedSize int) {
...
func assertValidTreeNode[TKey, TValue comparable](t *testing.T, node *Node[TKey, TValue], expectedEntries int, expectedChildren int, keys []TKey, hasParent bool) {
...
func BenchmarkBTreeGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	tree := NewWithIntComparator[int, struct{}](128)
	for n := 0; n < size; n++ {
		tree.Put(n, struct{}{})
	}
	b.StartTimer()
