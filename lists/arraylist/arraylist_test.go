...
func TestListRemove(t *testing.T) {
	...
	list.Add("b", "c")
	list.Remove(2)
	if actualValue, ok := list.Get(2); actualValue != *new(string) || ok {
		t.Errorf("Got %v expected %v", actualValue, nil)
	}
	list.Remove(1)
	list.Remove(0)
...
func TestListGet(t *testing.T) {
	...
	if actualValue, ok := list.Get(3); actualValue != *new(string) || ok {
		t.Errorf("Got %v expected %v", actualValue, nil)
	}
	list.Remove(0)
...
func TestListContains(t *testing.T) {
	...
	if actualValue := list.Contains(*new(string)); actualValue != false {
		t.Errorf("Got %v expected %v", actualValue, false)
	}
