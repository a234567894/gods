...
func TestMapFloor(t *testing.T) {
	...
	for _, test := range tests1 {
		...
		actualFound := actualKey != *new(int) && actualValue != *new(string)
...
func TestMapFind(t *testing.T) {
	...
	if foundKey != *new(string) || foundValue != *new(int) {
...
func TestMapChaining(t *testing.T) {
	...
	if actualValue, found := chainedMap.Get("aa"); actualValue != *new(int) || found {
...
func TestMapSerialization(t *testing.T) {
	...
	m := NewWithStringComparator[string, float32]()
	m.Put("a", 1.0)
	m.Put("b", 2.0)
	m.Put("c", 3.0)
...
func BenchmarkTreeMapGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	m := NewWithIntComparator[int, struct{}]()
	for n := 0; n < size; n++ {
		m.Put(n, struct{}{})
	}
	b.StartTimer()
